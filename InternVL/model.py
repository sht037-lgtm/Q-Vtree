...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _get_attn_scores(self, input_embeds, attention_mask, img_positions, text_positions):
        ...
        scores_list = []
        for i in [20, 27]:
            layer_attn = out.attentions[i]  # [1, heads, seq, seq]
            vp = img_positions.to(layer_attn.device)
            qp = text_positions.to(layer_attn.device)
            # text tokens attending to vision tokens: mean over heads and query positions
            attn = layer_attn[0, :, :, vp][:, qp, :]  # [heads, Q, N_img]
            scores_list.append(attn.mean(0).mean(0).cpu().float())
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        # only use content tile tokens (ignore thumbnail) for scoring
        n_content_tokens = num_content_tiles * N_per_tile
        vis_positions = vis_positions_all[:n_content_tokens]

        # text positions: tokens after all image tokens to end of sequence
        img_end = vis_positions_all[-1].item()
        seq_len = input_ids.shape[1]
        text_positions = torch.arange(img_end + 1, seq_len, dtype=torch.long)

        A_q = self._get_attn_scores(input_embeds_full, attention_mask, vis_positions, text_positions)
        ...
        A_generic = self._get_attn_scores(gen_emb, gen_mask, vis_positions, generic_text_positions)
        scores_all = A_q / (A_generic + 1e-8)
        scores_all = (scores_all - scores_all.min()) / (scores_all.max() - scores_all.min() + 1e-6)

        # ---- stitch content tiles into full spatial map (ignore thumbnail) ----
        scores_tiled = scores_all.view(num_content_tiles, grid_h, grid_w)
        rows = []
        ...
        g1d = torch.exp(-ax ** 2 / (2 * sigma ** 2)) / torch.exp(-ax ** 2 / (2 * sigma ** 2)).sum()
