...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        # text token positions relative to a single-tile sequence
        # single tile sequence: [prefix | tile_img_tokens | suffix]
        # prefix: input_ids[:, :img_start]
        # tile img tokens: N_per_tile tokens
        # suffix: input_ids[:, img_end+1:]
        ...
        # positions within single-tile sequence
        ...
        # ---- per-tile forward: accumulate layer scores ----
        # layer_tile_scores[layer_idx] = list of [N_per_tile] tensors, one per content tile
        ...
        suffix_emb = emb_layer(suffix_ids.to(device))   # [1, T, D]

        for tile_idx in range(num_content_tiles):
            tile_emb = vit_embeds[tile_idx].unsqueeze(0)            # [1, N_per_tile, D]
            tile_input_emb = torch.cat([prefix_emb, tile_emb, suffix_emb], dim=1)  # [1, tile_seq_len, D]
            tile_attn_mask = torch.ones(1, tile_seq_len, dtype=attention_mask.dtype, device=device)

            with torch.no_grad():
                tile_out = self.language_model(
                    inputs_embeds=tile_input_emb,
                    attention_mask=tile_attn_mask,
                    output_attentions=True,
                    output_hidden_states=False,
                    return_dict=True,
                    use_cache=False,
            ...
            for layer_idx in self.ABLATION_LAYERS:
                layer_attn = tile_out.attentions[layer_idx]          # [1, heads, S, S]
                vp = tile_vis_positions.to(layer_attn.device)
                qp = tile_text_positions.to(layer_attn.device)
                attn = layer_attn[0, :, :, vp][:, qp, :]            # [heads, Q, N_per_tile]
                score = attn.mean(0).mean(0).cpu().float()           # [N_per_tile]
