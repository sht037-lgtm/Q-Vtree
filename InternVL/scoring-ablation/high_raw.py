...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        for layer_idx in self.ABLATION_LAYERS:
            ...
            scores_raw = (A_q_l - A_q_l.min()) / (A_q_l.max() - A_q_l.min() + 1e-6)

            smooth_map, per_tile = _stitch_and_smooth(scores_raw)
        ...
        mean_scores = (
            self._extract_layer_attn(lm_out_q, 19, vis_positions, text_positions) +
