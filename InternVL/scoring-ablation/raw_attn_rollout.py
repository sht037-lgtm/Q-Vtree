...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _compute_rollout(self, lm_out, target_layer_idx, img_positions, text_positions):
        """
        Attention rollout from layer 0 up to target_layer_idx (inclusive).

        At each layer we build the full seq×seq attention matrix (mean over heads),
        add residual (0.5*A + 0.5*I), row-normalise, then chain-multiply.
        Finally we read out text→vision slice and average over text positions.

        Returns [N_img] scores.
        """
        seq_len = lm_out.attentions[0].shape[-1]
        device  = lm_out.attentions[0].device

        # start with identity
        rollout = torch.eye(seq_len, device=device, dtype=torch.float32)  # [S, S]

        for i in range(target_layer_idx + 1):
            # mean over heads → [S, S]
            A = lm_out.attentions[i][0].mean(0).float()   # [S, S]
            # residual: 0.5*A + 0.5*I
            A = 0.5 * A + 0.5 * torch.eye(seq_len, device=device, dtype=torch.float32)
            # row-normalise
            A = A / (A.sum(dim=-1, keepdim=True) + 1e-8)
            rollout = A @ rollout   # [S, S]

        vp = img_positions.to(device)
        qp = text_positions.to(device)
        # text rows → vision cols: [Q, N_img] → mean over Q → [N_img]
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        for layer_idx in self.ABLATION_LAYERS:
            scores_roll = self._compute_rollout(lm_out_q, layer_idx, vis_positions, text_positions)
        ...
        # ---- final scoring: rollout up to layer 27 ----
        mean_scores = self._compute_rollout(lm_out_q, 27, vis_positions, text_positions)
