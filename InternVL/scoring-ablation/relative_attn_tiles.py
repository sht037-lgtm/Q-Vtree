...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        # ---- extract text-only input_ids (no image tokens) for per-tile forward ----
        ids_flat = input_ids.reshape(-1)
        sel_mask = (ids_flat == self.img_context_token_id)
        vis_positions_all = sel_mask.nonzero(as_tuple=True)[0].cpu()
        img_start = vis_positions_all[0].item()
        img_end   = vis_positions_all[-1].item()

        prefix_ids = input_ids[:, :img_start]       # [1, P]
        suffix_ids = input_ids[:, img_end + 1:]     # [1, T]
        N_text     = suffix_ids.shape[1]
        tile_seq_len = img_start + N_per_tile + N_text

        tile_vis_positions  = torch.arange(img_start, img_start + N_per_tile, dtype=torch.long)
        tile_text_positions = torch.arange(img_start + N_per_tile, tile_seq_len, dtype=torch.long)

        # ---- generic suffix embeddings (replace question text with generic prompt) ----
        ...
        prefix_emb   = emb_layer(prefix_ids.to(device))                              # [1, P, D]
        suffix_emb_q = emb_layer(suffix_ids.to(device))                              # [1, T, D]
        generic_emb  = emb_layer(self._generic_token_ids.to(device))                 # [1, G, D]

        N_generic      = generic_emb.shape[1]
        generic_seq_len = img_start + N_per_tile + N_generic
        generic_vis_positions  = torch.arange(img_start, img_start + N_per_tile, dtype=torch.long)
        generic_text_positions = torch.arange(img_start + N_per_tile, generic_seq_len, dtype=torch.long)

        # ---- per-tile relative attention ----
        layer_tile_scores = {layer_idx: [] for layer_idx in self.ABLATION_LAYERS}

        for tile_idx in range(num_content_tiles):
            tile_emb = vit_embeds[tile_idx].unsqueeze(0)   # [1, N_per_tile, D]

            # question forward
            q_input = torch.cat([prefix_emb, tile_emb, suffix_emb_q], dim=1)
            q_mask  = torch.ones(1, tile_seq_len, dtype=attention_mask.dtype, device=device)
            with torch.no_grad():
                out_q = self.language_model(
                    inputs_embeds=q_input,
            ...
            # generic forward
            g_input = torch.cat([prefix_emb, tile_emb, generic_emb], dim=1)
            g_mask  = torch.ones(1, generic_seq_len, dtype=attention_mask.dtype, device=device)
            with torch.no_grad():
                out_g = self.language_model(
                    inputs_embeds=g_input,
            ...
            for layer_idx in self.ABLATION_LAYERS:
                # question attention
                attn_q = out_q.attentions[layer_idx]
                vp = tile_vis_positions.to(attn_q.device)
                qp = tile_text_positions.to(attn_q.device)
                A_q = attn_q[0, :, :, vp][:, qp, :].mean(0).mean(0).cpu().float()   # [N_per_tile]

                # generic attention
                attn_g = out_g.attentions[layer_idx]
                vp_g = generic_vis_positions.to(attn_g.device)
                qp_g = generic_text_positions.to(attn_g.device)
                A_g = attn_g[0, :, :, vp_g][:, qp_g, :].mean(0).mean(0).cpu().float()  # [N_per_tile]

                # relative score
                score = A_q / (A_g + 1e-8)
                layer_tile_scores[layer_idx].append(score)
        ...
        def _stitch_and_smooth(scores_flat):
            ...
            smooth = (full_map - full_map.min()) / (full_map.max() - full_map.min() + 1e-6)
            per_tile = []
            for row in range(ratio_h):
            ...
            if has_thumbnail:
                thumb_raw = scores_tiled[-1].float()
                thumb_s = (thumb_raw - thumb_raw.min()) / (thumb_raw.max() - thumb_raw.min() + 1e-6)
                per_tile.append(thumb_s.view(-1))
            return smooth, per_tile

        # ---- stitch per-tile scores into global maps ----
        self._debug_layer_score_maps = {}
        layer_smoothed_tiles = {}
        for layer_idx in self.ABLATION_LAYERS:
            # cat all tile scores → [num_content_tiles * N_per_tile]
            scores_flat = torch.cat(layer_tile_scores[layer_idx], dim=0)
            smooth_map, per_tile = _stitch_and_smooth(scores_flat)
            self._debug_layer_score_maps[layer_idx] = smooth_map
            layer_smoothed_tiles[layer_idx] = per_tile

        # ---- final scoring: mean of layers [19, 27] ----
        scores_19 = torch.cat(layer_tile_scores[19], dim=0)
        scores_27 = torch.cat(layer_tile_scores[27], dim=0)
        mean_scores = (scores_19 + scores_27) / 2
