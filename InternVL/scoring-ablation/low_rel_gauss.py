...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        def _smooth_map(scores_flat):
            m = scores_flat.reshape(grid_h, grid_w).float()
            m = F.conv2d(m.unsqueeze(0).unsqueeze(0), kernel, padding=ks // 2).squeeze()
            m = (m - m.min()) / (m.max() - m.min() + 1e-6)
            return m

        # ---- 7-layer relative attention on thumbnail ----
        self._debug_layer_score_maps = {}
        for layer_idx in self.ABLATION_LAYERS:
            ...
            self._debug_layer_score_maps[layer_idx] = _smooth_map(scores_rel)

        # final score = mean of layer 19 and 27
        final_map = (self._debug_layer_score_maps[19] + self._debug_layer_score_maps[27]) / 2
        final_map = (final_map - final_map.min()) / (final_map.max() - final_map.min() + 1e-6)
        self._debug_full_score_map = final_map

        # patch_scores for QVTree: broadcast final map to all content tiles
        smoothed = [final_map.reshape(-1) for _ in range(num_content_tiles)]
        if has_thumbnail:
            smoothed.append(final_map.reshape(-1))
    ...
    def infer(
        ...
        # ── small image for scoring (longest edge = input_size, 1 tile + thumbnail) ──
        scale_factor = input_size / max(orig_w, orig_h)
        small_w = max(1, int(orig_w * scale_factor))
        small_h = max(1, int(orig_h * scale_factor))
        image_small = image.resize((small_w, small_h), Image.BILINEAR)

        tiles_small, best_ratio_small = dynamic_preprocess(
            image_small,
            min_num=1,
            max_num=1,
        ...
        pixel_values_small = torch.stack([transform(t) for t in tiles_small]).to(
            dtype=dtype, device=device,
        )
        # best_ratio for scoring is always (1,1)
        best_ratio = best_ratio_small
        ratio_w, ratio_h = best_ratio
        num_content_tiles = ratio_w * ratio_h
        ...
        # ── Pass 1: scoring forward on small image ──
        query1, eos_token_id = _build_query(len(tiles_small))
        ...
        self.generate(
            pixel_values=pixel_values_small,
        ...
        # ── LPD: use final score map to select regions in original image ──
        global_h = GRID_SIZE
        global_w = GRID_SIZE
        hidden_dim = self.config.llm_config.hidden_size

        score_map = self._debug_full_score_map
        if score_map is None:
            global_scores = torch.zeros(global_h * global_w)
        else:
            global_scores = score_map.reshape(-1).cpu().float()

        # QVTree selection on global grid
        ...
        # map small image grid coords → original image pixel coords
