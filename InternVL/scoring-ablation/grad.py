...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        # ---- grad-cam scoring (requires grad, no torch.no_grad context) ----
        layer_scores = self._compute_grad_cam_scores(
            input_embeds_full, attention_mask, vis_positions, text_positions
        )

        # ---- per-layer score maps ----
        self._debug_layer_score_maps = {}
        layer_smoothed_tiles = {}
        for layer_idx in self.ABLATION_LAYERS:
            s = layer_scores[layer_idx]
            s = (s - s.min()) / (s.max() - s.min() + 1e-6)
            smooth_map, per_tile = _stitch_and_smooth(s)
    ...
    def generate(
        ...
        with torch.no_grad():
            return self.language_model.generate(
                inputs_embeds=input_embeds_final.detach(),
                attention_mask=attention_mask,
                generation_config=generation_config,
