...
class InternVLChatModelWithTree(InternVLChatModel):
    ...
    def _compute_relative_rollout(self, lm_out_q, lm_out_g, target_layer_idx,
                                   img_positions, text_positions):
        """
        Relative attention rollout up to target_layer_idx.

        At each layer:
          1. Compute a question-vs-generic relative text->vision weight vector
          2. Re-weight the vision columns of A_q with that vector
          3. Apply standard residual rollout on the re-weighted matrix

        This variant is intentionally used instead of full-matrix relative
        rollout because the question and generic prompts have different text
        lengths, so their full attention matrices are not directly aligned.

        Returns [N_img] scores.
        """
        seq_len_q = lm_out_q.attentions[0].shape[-1]
        device = lm_out_q.attentions[0].device

        vp = img_positions.to(device)
        qp = text_positions.to(device)

        rollout = torch.eye(seq_len_q, device=device, dtype=torch.float32)

        for i in range(target_layer_idx + 1):
            A_q = lm_out_q.attentions[i][0].mean(0).float()   # [S, S]
            A_g = lm_out_g.attentions[i][0].mean(0).float()   # [S_g, S_g]
            seq_len_g = A_g.shape[-1]
            vp_g = vp.clamp(max=seq_len_g - 1)
            qp_g = qp.clamp(max=seq_len_g - 1)

            # Relative weighting is defined only on comparable text->vision paths.
            attn_q_vis = A_q[qp][:, vp].mean(0)              # [N_img]
            attn_g_vis = A_g[qp_g][:, vp_g].mean(0)          # [N_img]
            rel_weights = attn_q_vis / (attn_g_vis + 1e-8)   # [N_img]
            rel_weights = rel_weights / (rel_weights.mean() + 1e-8)

            A_mod = A_q.clone()
            A_mod[:, vp] = A_mod[:, vp] * rel_weights.unsqueeze(0)
            A_mod = 0.5 * A_mod + 0.5 * torch.eye(seq_len_q, device=device, dtype=torch.float32)
            A_mod = A_mod / (A_mod.sum(dim=-1, keepdim=True) + 1e-8)
            rollout = A_mod @ rollout

        scores = rollout[qp][:, vp].mean(0).cpu().float()
        return scores

    def _get_attn_scores(self, input_embeds, attention_mask, img_positions, text_positions):
        """Original scoring path: mean of layers [19, 27]. Kept for non-ablation use."""
    ...
    def _score_and_select(self, vit_embeds, input_embeds_full, attention_mask,
        ...
        def _stitch_and_smooth(scores_flat):
            """Stitch per-tile scores into global map, normalize, return (score_map, per_tile)."""
        ...
        # ---- per-layer relative attention rollout ----
        # _debug_layer_score_maps: dict {layer_idx: score_map [H_global, W_global]}
        self._debug_layer_score_maps = {}

        layer_smoothed_tiles = {}
        for layer_idx in self.ABLATION_LAYERS:
            scores_roll = self._compute_relative_rollout(
                lm_out_q, lm_out_g, layer_idx, vis_positions, text_positions
            )
            scores_roll = (scores_roll - scores_roll.min()) / (scores_roll.max() - scores_roll.min() + 1e-6)

            smooth_map, per_tile = _stitch_and_smooth(scores_roll)
            self._debug_layer_score_maps[layer_idx] = smooth_map
            layer_smoothed_tiles[layer_idx] = per_tile

        # ---- final scoring: relative rollout up to layer 27 ----
        mean_scores = self._compute_relative_rollout(
            lm_out_q, lm_out_g, 27, vis_positions, text_positions
        )
