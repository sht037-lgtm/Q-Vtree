...
def infer_from_pil(model, image: Image.Image, question: str,
                   max_new_tokens: int = 16) -> str:
    """Run InstructBLIP inference from a PIL image."""
...
# V-Star
# =========================================================
def run_vstar_inference_instructblip(
    model,
    ...
    model_type: str = "base_instructblip",
    run_name: str | None = None,
):
    """
    model_type: "base_instructblip"  – standard InstructBLIP inference
                "tree_instructblip"  – QVTree + LPD pipeline
    """
    if model_type not in ["base_instructblip", "tree_instructblip"]:
    ...
    use_tree = (model_type == "tree_instructblip")

    with open(output_path, "w", encoding="utf-8") as fout:
        for sample in tqdm(samples, desc=f"Running V-Star [{model_type}]", miniters=10):
            ...
            try:
                ...
                if use_tree:
                    if (hasattr(model, '_debug_num_selected_tokens')
                            and model._debug_num_selected_tokens):
...
# HR-Bench
# =========================================================
def run_hrbench_inference_instructblip(
    model,
    split: str = "4k",
